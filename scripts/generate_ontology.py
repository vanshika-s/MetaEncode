...
from rdflib import RDF, RDFS, BNode, ConjunctiveGraph, Namespace, exceptions
from rdflib.collection import Collection

from .manual_slims import slim_shims
from .ntr_terms import ntr_assays, ntr_biosamples
...
class Inspector(object):
    ...
    def __getAllClasses(
        ...
        def addIfYouCan(x, mydict):
            if excludeRDF_OWL:
                if (
                    x.startswith("http://www.w3.org/2002/07/owl#")
                    or x.startswith("http://www.w3.org/1999/02/22-rdf-syntax-ns#")
                    or x.startswith("http://www.w3.org/2000/01/rdf-schema#")
                ):
                    return mydict
            if x not in mydict:
                mydict[x] = None
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Get Uberon, EFO, OBI, and CLO ontologies and generate the JSON file",
        epilog=EPILOG,
                        ...
                        for o1 in data.rdfGraph.objects(c, IntersectionOf):
                            ...
                            if HUMAN_TAXON in col_list:
                                if PART_OF in col_list:
                                    for subC in data.rdfGraph.objects(
                                        c, RDFS.subClassOf
                                    ):
                                        ...
                                        terms[term_id]["part_of"].append(
                                            splitNameFromNamespace(subC)[0].replace(
                                                "_", ":"
                                            )
                                        )
                            ...
                            if o.__str__() == PART_OF:
                                ...
                                for o1 in data.rdfGraph.objects(parent, SomeValuesFrom):
                                    if not isBlankNode(o1):
                                        ...
                                        for o2 in data.rdfGraph.objects(
                                            o1, IntersectionOf
                                        ):
                                            for o3 in data.rdfGraph.objects(
                                                o2, RDF.first
                                            ):
                                            ...
                                            for o3 in data.rdfGraph.objects(
                                                ...
                                                for o4 in data.rdfGraph.objects(
                                                    o3, RDF.first
                                                ):
                                                    for o5 in data.rdfGraph.objects(
                                                        o4, SomeValuesFrom
                                                    ):
                                                        for o6 in data.rdfGraph.objects(
                                                            o5, IntersectionOf
                                                        ):
                                                            for (
                                                                o7
                                                            ) in data.rdfGraph.objects(
                                                                o6, RDF.first
                                                            ):
    ...
    for term in terms:
        terms[term]["data"] = list(
            set(terms[term]["parents"])
            | set(terms[term]["part_of"])
            | set(terms[term]["derives_from"])
            | set(terms[term]["achieves_planned_objective"])
        )
    ...
    for term in terms:
        ...
        del (
            terms[term]["id"],
            terms[term]["data"],
            terms[term]["data_with_develops_from"],
        )
